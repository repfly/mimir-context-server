...
class RetrievalService:
    ...
    async def search(
        ...
        *,
        ...
        if mode == "none" or flat:
            seeds = self._flat_search(query_embedding, where=where_filter)
        else:
    ...
    def _flat_search(
        ...
        # We need the node objects — defer to caller to resolve from graph
        return [(VectorSearchResultNode(r.id, r.score), r.score) for r in results]
...
class VectorSearchResultNode(Node):
    """Lightweight placeholder node used during flat search before graph resolution."""

    def __init__(self, node_id: str, score: float) -> None:
        super().__init__(
            id=node_id,
            repo=node_id.split(":")[0] if ":" in node_id else "",
